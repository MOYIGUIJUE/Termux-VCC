...
set LANG=zh_CN
doskey cat=type $*
doskey clear=echo off $t cls $t title $t modes 68 15 $t colors 0x70 $t more +%splitline% %0 $t colors 0x07 $t modes 70 15 $t set .= ^& set /p ".=%used%@%COMPUTERNAME%[%%cd%%]$ " ^& call %%.%% ^& set .=^& echo on
doskey cp=copy $*
doskey mv=move $*
doskey pwd=cd
...
doskey ca=set /a result=$* ^& echo;%result%
call :random 
more +%splitline% %0
colors 0x07
modes 70 15
