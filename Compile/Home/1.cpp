...
{
    static TCHAR szAppName[] = TEXT("matrix");//窗口类名
    HWND hwnd;
    MSG msg;
    WNDCLASS wndclass = {0};
    ...
    hwnd = CreateWindow(szAppName, TEXT("GUI"),
        WS_BORDER | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX |WS_MINIMIZEBOX,
        400,100,800,600,NULL, NULL, hInstance,NULL);
    ShowWindow(hwnd, SW_SHOW); //最大化显示 
    UpdateWindow(hwnd);
    //ShowCursor(FALSE); //隐藏鼠标光标 
    //srand((int)GetCurrentTime()); //初始化随机数发生器 
    while (GetMessage(&msg, NULL, 0, 0))
    {
        TranslateMessage(&msg);//将虚拟键值消息转换未字符消息
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		printf("线程id:%d",GetCurrentThreadId());
		g_mouseHook = SetWindowsHookEx(WH_MOUSE,MouseProc,NULL,GetCurrentThreadId());
		g_keyBoard = SetWindowsHookEx(WH_KEYBOARD,Keyboard,NULL,GetCurrentThreadId());
	...
	case WM_LBUTTONDOWN:
		...
		MessageBox(hwnd,TEXT("gfd"),"fhdg",MB_OK);
		break;
	}
    /* HDC hdc;
