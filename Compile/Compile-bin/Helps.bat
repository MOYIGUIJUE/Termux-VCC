...
echo;	ºóÃæÌí¼Ó°²×°Â·¾¶,»ò²»Ìí¼Óµ¯³öÎÄ¼ş¼ĞÑ¡Ôñ¿ò
echo;	³õ´Î°²×°ÔÚµ¯³öµÄ¹ÜÀíÔ±ºÚ¿òÖĞÊäÈëall,»Ø³µ,¹Ø±Õ
echo;^> exec [path]
echo;	ºóÃæÌí¼Ó°²×°Â·¾¶,ÕâÀï°üº¬3¸ö×¢²áÏî
echo;	gccpath¡¢ÓÒ¼ünotepad++¡¢ÓÒ¼ütermux
	call :command
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [2]±àÒëµÚÒ»¸öC/C++³ÌĞò
echo;^> com 1.cpp 
echo;	1.cppÈç²»´æÔÚÔò»á´´½¨Ò»¸ö·¶Àı1.cpp
echo;	È»ºóÔÙ´ÎÊäÈë com 1.cpp ¾Í»á±àÒë
echo;^> com -c [path\filename]
echo;	ºóÃæÌí¼ÓÎÄ¼şÃû,Èç¹û²»´æÔÚÒ²»á´´½¨
echo;	²»Ìí¼Ó,½«»áÓĞÒ»¸ö¿ØÖÆÌ¨½çÃæ¿É¹©Ñ¡Ôñ
	call :command
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [3]´ò¿ªcppÎÄ¼ş
echo;^> note 1.cpp 
echo;	»áÓÃnotepad++´ò¿ª1.cpp
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [4]²é¿´°ïÖúĞÅÏ¢
echo;^> [command] {[--help][/?][-h][]}
echo;	commandÊÇÃüÁî,ºóÃæÊÇ³£¼ûµÄ²ÎÊı
echo;	±¾³ÌĞòÀ©Õ¹ÃüÁî´ó²¿·ÖÊÇ²»´ø²ÎÊıÏÔÊ¾°ïÖúĞÅÏ¢
echo;	±ÈÈçls --help ¡¢dir /?¡¢vcc ¡¢cs -h
echo;^> ll
echo;	Ò»°ãÂÌÉ«ÊÇ¿ÉÖ´ĞĞÎÄ¼ş,À¶É«ÎªÎÄ¼ş¼Ğ
echo;	¿ØÖÆÌ¨Ô­À´µÄÑÕÉ«ÎªÆÕÍ¨ÎÄ¼ş
	call :command
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [5]wsl/bashÊÇwindows×Ô´øµÄlinux×ÓÏµÍ³
echo;^> bash
echo;	ÕâÀïÊÇÊ¹ÓÃubantuÏµÁĞ
echo;^> wsl
echo;	Õâ¸öºÍÉÏÃæµÄÃüÁîÒ»Ñù
echo;	¼òµ¥Ñ§Ï°Ò»ÏÂlinuxÃüÁî,Ğ´Ò»Ğ©bash½Å±¾
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [6]»ù±¾½Ì³ÌÒÑ¾­½áÊø,ÏÂÃæÊÇ½ø½×Íæ·¨
echo;^> vcc -
echo;	½øÈëÀ©Õ¹³ÌĞòÎÄ¼şÄ¿Â¼,ÊäÈëll²é¿´¿ÉÖ´ĞĞÎÄ¼şÃüÁî
echo;	ÕâÀïµÄËùÓĞÃüÁî¾ÍÊÇµÚ¶ş½×¶ÎµÄÑ§Ï°ÄÚÈİ
echo;	×îÉÏ²ãµÄtermux.batÄÚ»¹ÓĞÒ»Ğ©ºêÃüÁî
echo;	µÚÈı½×¶Î¾ÍÊÇDIYÕâ¸ö¼¯ºÏÁË
	call :command
	if errorlevel 404 ( cls ) else pause >nul &cls
echos 0x0e [7]µ½ÕâÀï¾Í»ù±¾½áÊøÁË,ÏÂÃæµÄÌî´Ê¿ÉÒÔÊÔÒ»ÊÔ,°´ESC½áÊø
echo;[·­Òë]Âé·³Ã»À´ÕÒÄã£¬¾Í±ğÈ¥×ÔÕÒÂé·³¡£
words "1. Never trouble trouble" "till" "trouble troubles you¡£"
echo;	                 ----
echo;	µÚÒ»¡¢ËÄ¸ötroubleÊÇ¶¯´Ê£¬µÚ¶ş¡¢Èı¸ötroubleÊÇÃû´Ê¡£
echo;[µ¥´ÊÊÍÒâ]till
echo;	v.¸û×÷£»ÀçµØ
...
cls
echos 0x0e [8]ClearË¢ĞÂ
echo;^> start https://gitee.com/cctv3058084277/main 
echo;	²é¿´²¢ÏÂÔØ×îĞÂ°æ±¾
echo;^> clear
echo;	»Øµ½³õÊ¼½çÃæ
echo;^> exit /b	»òÕß	^> exit
echo;	Äã·¢ÏÖ´Ë½Ì³Ì½áÊøºó,ÌáÊ¾·ûÓÖÃ»ÓĞÑÕÉ«ÁË
echo;	Äã¿ÉÒÔÊäÈëÒÔÉÏÃüÁî½âËøËü
cmdow @ /NOT
for /f %%i in ('CWnd find /!') do (
	CWnd enable %%i min
	CWnd enable %%i max
...
REM echo;	¿ÉÒÔ¼òµ¥¿ìËÙĞ´Ò»µãÅú´¦Àíµ±³¡Ö´ĞĞ
REM echo;	ctrl+z»Ø³µ±£´æ
REM echo;	ÏÂÃæÊÇÄ£·ÂmysqlÀïÃæµÄÊäÈëÄ£Ê½,¼ì²âµ½;½áÊø
REM echo;
